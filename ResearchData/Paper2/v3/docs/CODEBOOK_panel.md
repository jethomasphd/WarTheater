# Codebook — `data/panel_daily.csv` (Paper 2, v3)
...
| `snap_iranian_killed` | float | Dashboard cumulative Iranian killed snapshot, linearly interpolated between observed days (raw coverage 107/170). Carries the Day-57 re-anchoring (9,226 → 3,375); see the manuscript Box 1 and Table 9. |
...
One row per screened health-system row (82 rows; see METHODS §3). Rows are **retained even
...
## Plain-language notes on the variables this paper uses most

- `iran_civ`, `killed_total`: the outcomes. Daily counts. Adding `iran_civ` over
  all days gives 543; adding `killed_total` gives 6,213.
- `strikes`: the exposure. A count of distinct places struck that day. Days with
  `strikes > 0` are "strike days" (89 of 170).
- `facil_damage_pct`: the primary damage measure, 0–100. It is 10% on Day 15,
  50% on Day 26, 75% on Day 33, 99% on Day 39, and 99–100% from then on.
- `hssi_pct`: the secondary damage measure, 0–100, from 21 audited events.
- `cum_iran_daily`, `cum_leb_daily`: the daily series added up. At Day 170 they
  are 3,166 and 2,993 (the lower bounds of the direct toll).
- `snap_iranian_killed`, `snap_lebanese_killed`: the dashboard counters,
  interpolated between observed days. At Day 170 they are 3,636 and 4,308 (the
  upper bounds).

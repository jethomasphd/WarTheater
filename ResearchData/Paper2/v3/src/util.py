...
Version-3 note
--------------
This is the v3 (plain-language, three-tool) edition. It lives at
`ResearchData/Paper2/v3/` and writes its own panel, tables, and figures under
that directory, but it reads the *same* frozen v1.2 release that the parent
paper and v2 read. The data-building code in this file (the health-system
register, its audit trail, the facility-damage benchmarks, and the daily panel)
is carried over from v2 unchanged, so the panel is identical. What changes in
v3 is the analysis layer (scripts 01-05): only three tools are used (means
comparison, correlation, regression), and every result is written to be read
by someone with basic graduate statistics training.
"""
...
PAPER_DIR = SRC_DIR.parent                       # ResearchData/Paper2/v3
RESEARCH_DIR = PAPER_DIR.parent.parent           # ResearchData (two up: v3 -> Paper2 -> ResearchData)

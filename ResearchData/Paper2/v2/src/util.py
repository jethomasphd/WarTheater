...
Version-2 note
--------------
This is the v2 (student-findings) edition. It lives one directory deeper than
the parent paper (`ResearchData/Paper2/v2/`) and writes its own panel, tables,
and figures under that directory, but it reads the *same* frozen v1.2 release,
so every number regenerated here is identical to the parent paper's — the v2
edition re-narrates the same evidence around three findings, it does not
re-analyse different data.
"""
...
PAPER_DIR = SRC_DIR.parent                       # ResearchData/Paper2/v2
RESEARCH_DIR = PAPER_DIR.parent.parent           # ResearchData (two up: v2 -> Paper2 -> ResearchData)

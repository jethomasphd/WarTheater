...
**Current version: v1.1** (Day 0–100, 2026-02-27 → 2026-06-07). Released 2026-06-09.
For the change history and the frozen v1.0 release, see `CHANGELOG.md` and `releases/`.
...
- **Conflict events**: 2026-02-28 (Day 1) to 2026-06-07 (Day 100)
- **Day 0** (2026-02-27) = pre-war baseline reference date

## Dataset Summary (v1.1)
...
| Event types | 44 unique subcategories |
| Source files | 15 extracted JSON files from `public/data/` |
| Date range | 2026-01-02 to 2026-06-07 |
...
|--------|------|-------------|
| FINANCIAL | 879 | Oil prices, market indices, war costs, tanker transits, baselines, alt routes |
| STRIKE | 790 | US/Israeli offensive strikes (exploded by target × active day) + offensive timeline/retaliation-file events |
| HUMANITARIAN | 561 | Daily casualty reports (5 factions × 100 days) + infrastructure damage + timeline |
| RETALIATION | 551 | Iranian/Hezbollah/Houthi/proxy attacks + maritime attack summary |
| DIPLOMATIC | 361 | Political, diplomatic, sanctions, congressional, and domestic events |
| OTHER | 238 | Daily briefings, aggregate snapshots, historical comparisons, summaries |
| MILITARY | 123 | General military events, base references, operational losses, ground ops |
| NAVAL | 80 | Fleet deployments, Strait of Hormuz events, maritime incidents, strategic geography |
...
|------------|-------------------|-------------|
| `timeline-events.json` | 937 | Master conflict timeline across all domains |
| `strikes-iran.json` | 735 | US/Israeli strike locations on Iran (105 locations, exploded) |
| `strikes-retaliation.json` | 421 | Retaliation / cross-front / maritime / operational-loss events (196 locations) |
| `casualties.json` | 500 | Daily estimated killed by faction (100 days × 5 factions) |
| `markets.json` | 312 | Market sector performance (78 trading days × 4 sectors) |
| `war-costs.json` | 203 | Daily war cost estimates (100) + tanker transit counts (103) |
| `oil-prices.json` | 178 | Brent and WTI crude oil prices (89 dates × 2) |
| `hero-stats.json` | 101 | Daily aggregate metric snapshots |
| `briefings/index.json` | 94 | Daily intelligence briefing headlines |
...
| `carriers.json` | 17 | Naval asset deployments (US, UK, France) |
| `global-bases.json` | 15 | US military base reference locations |
...
```
ResearchData/
├── iranwar_event_dataset.csv   # latest dataset (v1.1) — stable citation path
...
> IranWar.ai Event-Level Research Dataset (v1.1): OSINT Record of the 2026 US-Iran Conflict

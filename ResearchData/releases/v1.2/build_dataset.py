...
# Constants
# ---------------------------------------------------------------------------
DATASET_VERSION = "1.2"              # see CHANGELOG.md
...
def extract_strikes_iran() -> list[dict]:
    ...
    for rec in records:
        ...
        # v1.2: also fold `notes_addendum` (added after Day 100) so no source text is lost.
        ...
        if rec.get("notes_addendum"):
            _extra.append(f"Addendum: {rec['notes_addendum']}")
        if _extra:
...
# Retaliation/cross-front type maps (v1.1; extended v1.2). Day 30 had 5 types;
# Day 100 had ~32; Day 170 adds `houthi_proxy_attack` (Houthi USV/missile strikes on
# Red Sea shipping) -> RETALIATION/maritime_attack, consistent with the existing
# `drone_strike_on_commercial_vessel` and `houthi_attack` handling.
...
RET_TYPE_DOMAIN = {
    ...
    "houthi_attack": "RETALIATION", "houthi_proxy_attack": "RETALIATION",
...
RET_TYPE_EVENTTYPE = {
    ...
    "houthi_attack": "drone_attack", "houthi_proxy_attack": "maritime_attack",
...
def normalize_naval_type(raw: str | None) -> str:
    ...
    v1.2 adds `mine_countermeasures` (MCM ships) and `unmanned_surface_vessel` (USVs),
    which entered carriers.json after Day 100."""
    ...
    if ("mine" in t and "countermeasure" in t) or "mcm" in t:
        return "mine_countermeasures"
    if any(k in t for k in ("unmanned", "uncrewed", "usv")):
        return "unmanned_surface_vessel"
...
def extract_infrastructure() -> list[dict]:
    ...
    infra_type_map = {
        ...
        # v1.2: new category appeared after Day 100
        "Water/Desalination Facilities Struck": "water_infrastructure",
    }
...
VARIABLE_METADATA = {
    ...
    "day_of_conflict": ("Day of Conflict", "integer", "all", "Should not be missing",
                        "Day 0 = 2026-02-27 (pre-war baseline). Day 1 = 2026-02-28 (first strikes). Max grows each release; v1.2 covers Day 0-170."),
    ...
    "infrastructure_target_type": ("Infrastructure Target Type", "categorical", "strikes-iran.json; infrastructure.json", "Not applicable (non-strike or non-infrastructure event)",
                                   "Inferred from target descriptions. Values: military_base, civilian, nuclear_facility, oil_infrastructure, air_defense, communications, healthcare, power_grid, transportation, water_infrastructure."),
    ...
    "strike_verified": ("Strike Verified", "boolean", "strikes-retaliation.json", "Not applicable (non-retaliation event)",
                        "Whether the retaliation strike was verified in open-source reporting. True/False. 18 of 227 retaliation locations are unverified (verified=False)."),
...
def generate_codebook(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        # Valid values — auto-generate from data. Cap high enough to enumerate every
        # categorical in full (largest is event_type; 46 values as of v1.2) so the
        # codebook honors its own "see valid_values for the full enumerated list" note.
        if vtype == "categorical" and len(non_null) > 0:
            uniques = sorted(non_null.unique().tolist())
            valid = " | ".join(str(v) for v in uniques[:80])
